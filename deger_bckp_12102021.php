...
<?php
            if(isset($_POST["bul"])){
              ...
              {
                echo "nokta kodu seçildi!";

              //$sorgu = $connect->prepare("select count(*) from Botas_EBT_Integration ");
              //$sorgu->execute();
              //$say = $sorgu->fetchColumn();

              $tsql = "select * from Botas_EBT_Integration where RemoteTime='".$tarih." 08:00:00.000' and NoktaKodu='".$nokta_kodu."'";
              $tsql_c = "select count(*) from Botas_EBT_Integration where RemoteTime='".$tarih." 08:00:00.000' and NoktaKodu='".$nokta_kodu."'";
              $getResults = $connect->prepare($tsql);
              $getResults_c = $connect->prepare($tsql_c);
                $getResults->execute();
                $getResults_c->execute();
...
?>
    <table><table  border=1>
        <tr>
          ...
          <th>Tarih</th><th><input type="date" name="tarih"></th>
          <br><br>
    </table>
    <br>
