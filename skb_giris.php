...
<?php
              ...
              $tsql = "SELECT [Name],[NoktaKodu],[RemoteTime],[Consumption],[GunlukButceSkb],[GunlukGerceklesmeSkb]  FROM [TMX].[dbo].[Botas_EBT_Integration] where Name='".$nokta_name."' and RemoteTime between '".$tarih1."' and '".$tarih2."' order by RemoteTime";
              ...
              $tsql = "UPDATE [TMX].[dbo].[Botas_EBT_Integration] SET GunlukButceSkb=ROUND('".$skb."', 6), GunlukGerceklesmeSkb=ROUND('".$skb."', 6) where Name='".$nokta_name."' and RemoteTime between '".$tarih1."' and '".$tarih2."'";
              $getResults = $connect->prepare($tsql);
              $getResults->execute();
              //$results = $getResults->fetchAll(PDO::FETCH_BOTH);
              //foreach($results as $row){
