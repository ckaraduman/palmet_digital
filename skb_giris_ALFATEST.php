...
<?php
              ...
              $tsql = "UPDATE [TMX].[dbo].[Botas_EBT_Integration_cem] SET GunlukButceSkb=ROUND('".$skb."', 6), GunlukGerceklesmeSkb=ROUND('".$skb."', 6) where NoktaKodu='".$nokta_kodu."' and RemoteTime between '".$tarih1."' and '".$tarih2."'";
              $getResults = $connect->prepare($tsql);
              $getResults->execute();
              //$results = $getResults->fetchAll(PDO::FETCH_BOTH);
              foreach($results as $row){
